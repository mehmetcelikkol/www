...
<head>
  ...
  <style>
    ...
    .modal .content{ padding:12px 14px; font-size:12px; color:#0f172a; } /* KOYULAŞTIRILDI */
    .modal .content table{ width:100%; border-collapse:collapse; font-size:12px; background:#fff }
    .modal .content th, .modal .content td{ border:1px solid #e5e7eb; padding:6px 8px; text-align:left; color:#0f172a; } /* KOYULAŞTIRILDI */
    .modal .content th{ background:#f8fafc; font-weight:600; color:#0b1220; } /* KOYULAŞTIRILDI */
    .modal .muted{ color:#1f2937; font-size:11px; } /* KOYULAŞTIRILDI */
...
<form method="get" class="filter" novalidate>
  ...
  <script src="assets/js/grafikler.bundle.js" defer></script>
    ...
    function buildCharts(){
      ...
      devicesToShow.forEach(dev=>{
        ...
        btnSummary.addEventListener('click', (e)=>{
          e.preventDefault(); e.stopPropagation(); // EKLENDİ
          try{
            ...
            console.error('Özet Rapor açılırken hata:', err);
            alert('Özet rapor açılamadı. Konsolu kontrol edin.');
          }
        });
        btnDetail.addEventListener('click', (e)=>{
          ...
          try{
            ...
            console.error('Detaylı Rapor açılırken hata:', err);
            alert('Detaylı rapor açılamadı. Konsolu kontrol edin.');
          }
        });
    ...
    function showModal(title, htmlContent){
      ...
      const onEsc = (e)=>{ if(e.key==='Escape'){ close(); } }; // DEĞİŞTİ
      const close = ()=>{ 
        if(overlay && overlay.parentNode) overlay.parentNode.removeChild(overlay);
        document.removeEventListener('keydown', onEsc);         // DEĞİŞTİ
      };
      ...
      document.addEventListener('keydown', onEsc);              // DEĞİŞTİ
