...
void loop() {
  ...
  if (isnan(averageTemp) || isnan(averageHum)) {
    nansay++;
    //    Serial.println("NaN verisi alindi. Sayac: " + String(nansay));

    if (nansay >= 30) {
      //       Serial.println("NaN limiti asildi. Cihaz yeniden baslatiliyor...");
      yenidenbaslat();
    }
  } else {
    nansay = 0; // NaN tespit edilmezse sayaç sıfırlanır
  }
