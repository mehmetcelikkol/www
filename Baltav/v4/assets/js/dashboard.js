...
$(function(){
  function renderSummary(s){
    ...
    // Glow Entegrasyonu (Kritik Sayısına Göre)
    var $critCard = $('#criticalCount').closest('.card');
    if (s.critical > 0) {
        $critCard.addClass('glow-card-danger').removeClass('glow-card-warning');
    } else {
        $critCard.removeClass('glow-card-danger glow-card-warning');
    }

    var $commCard = $('#commErrors').closest('.card');
    if (s.comm_errors > 0) {
        $commCard.addClass('glow-card-warning');
    } else {
        $commCard.removeClass('glow-card-warning');
    }
  }
