...
class Auth {
    ...
    public function login($eposta, $parola, $beniHatirla = false) {
        ...
        if ($user && (password_verify($parola, $user['sifre_hash']) || $parola === $user['sifre_hash'])) { // Hash veya düz metin (geçici)
            ...
            log_action('USER_ACTION', 'Kullanıcı başarılı bir şekilde giriş yaptı.', ['kullanici_id' => $user['id'], 'eposta' => $eposta, 'rol' => $user['rol']]);
        ...
        log_action('WARN', 'Hatalı giriş denemesi.', ['eposta' => $eposta, 'parola_girisi_uzunlugu' => strlen($parola)]);
        return false;
    }
    ...
    public function logout() {
        log_action('USER_ACTION', 'Kullanıcı çıkış yaptı.');
        session_destroy();
